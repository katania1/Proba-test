...
# Импортируем роли из нашего модуля компонентов настроек
...
class ModelDatabaseDialog(QDialog):
    """
    Окно базы проверенных моделей (Dashboard).
    Отображает сводную таблицу всех протестированных LLM по провайдерам.
    """
        ...
        for provider_id, ui in self.parent_dialog.tabs_ui.items():
            ...
            for i in range(list_widget.count()):
                ...
                if status in ["ok", "error"]:
                    ...
                    if status == "error":
                        item_status.setToolTip(orig_item.toolTip())
