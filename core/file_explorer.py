...
class FileExplorerWidget(QWidget):
    ...
    open_time_machine_signal = pyqtSignal(str) # Сигнал для открытия машины времени
    ...
    def tree_context_menu(self, pos):
        ...
        files = []
        for i in indexes:
            abs_path = self.file_model.filePath(i)
        ...
        act_time_machine = None
        if len(indexes) == 1:
            menu.addSeparator()
            act_time_machine = menu.addAction("🕒 Машина Времени (История файла)")
        ...
        elif act_time_machine and action == act_time_machine:
            path = self.file_model.filePath(indexes[0])
            self.open_time_machine_signal.emit(path)
