...
# Импортируем наши новые модули (Лего-блоки)
from core.verification_worker import VerificationWorker
from core.model_db_dialog import (ModelDatabaseDialog, ROLE_NAME, ROLE_STATUS, 
                                  ROLE_NOTE, ROLE_LAST_TESTED, ROLE_IS_NEW)
from core.ui_components import create_standard_input, SecureInputWidget, RagKeyRowWidget

# ==========================================
# ОСНОВНОЕ ОКНО НАСТРОЕК С ВКЛАДКАМИ
# ==========================================
class APISettingsDialog(QDialog):
    ...
    def open_model_database(self):
        # Вызываем внешнее окно из импортированного модуля
        dlg = ModelDatabaseDialog(self)
        dlg.exec()

    def _add_emb_key_row(self, layout, key_data=None):
        """Добавляет кастомную строку RAG-ключа с использованием нового UI-компонента."""
        def remove_row(widget):
            layout.removeWidget(widget)
            widget.deleteLater()
            
        row_widget = RagKeyRowWidget(key_data, on_delete_callback=remove_row)
    ...
    def create_provider_tab(self, provider_id, default_tab_title, key_placeholder, has_base_url=True, is_custom=False, has_embedding_key=False):
        ...
        # --- БЛОК 1: Основной ключ (используем новый SecureInputWidget) ---
        ...
        ui['key'] = SecureInputWidget(placeholder=key_placeholder)
        creds_layout.addWidget(ui['key'])
        
        if has_base_url:
            creds_layout.addWidget(QLabel("Base URL:"))
            ui['url'] = create_standard_input(placeholder="https://api...")
        ...
        ui['search'] = create_standard_input(placeholder="🔍 Поиск по модели или примечанию...")
    ...
    def _get_provider_instance(self, provider_id):
        ...
        else: return OpenAIProvider(key, url)
    ...
    def load_settings(self):
        # Читаем основные ключи и расставляем по UI-компонентам
    ...
    def save_settings(self):
        ...
        for p_id, ui in self.tabs_ui.items():
            ...
            if not ui['is_custom']:
                ...
                # --- УМНОЕ СОХРАНЕНИЕ ПУЛА RAG КЛЮЧЕЙ ИЗ КОМПОНЕНТОВ ---
                if 'emb_keys_layout' in ui:
                    keys_list = []
                    for i in range(ui['emb_keys_layout'].count()):
                        row_w = ui['emb_keys_layout'].itemAt(i).widget()
                        if isinstance(row_w, RagKeyRowWidget):
                            k_val = row_w.get_data().get("key", "").strip()
                            if k_val:
                                keys_list.append(row_w.get_data())
