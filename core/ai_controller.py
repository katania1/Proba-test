...
class AIController(QObject):
    ...
    def execute_api_task(self, provider_id, selected_model):
        ...
        try:
            if provider_id == "OpenAI":
                ...
                custom_data = settings.value("custom_providers", "[]")
                ...
                key = found.get('key', '')
                url = found.get('url', '')
                if not key: self.mw.log_system(f"Внимание: Ключ для {found['name']} пуст (возможно, это локальная нейросеть).", color="#ffaa00")
                provider = OpenAIProvider(key, url)
    ...
    def request_ai_commit_message(self, diff_text):
        ...
        prompt = (
            ...
            "Используй только одинарные (') или елочки («»)."
        )
    ...
    def process_ai_response(self, raw_text):
        ...
        if result["status"] == "error":
            ...
            fix_prompt = (
                f"Твой предыдущий ответ вызвал фатальную ошибку: {result['error_message']}\n"
