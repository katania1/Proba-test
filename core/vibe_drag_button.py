...
class VibeDragButton(QPushButton):
    """
    Гибридная кнопка отправки v4.3 (Интеллектуальное сохранение имен файлов в тексте).
    """
    ...
    def start_vibe_drag(self):
        ...
        if not user_text:
            ...
            log_func("Введите запрос перед перетягиванием!", color="#ffaa00")
            return
        ...
        # Вывод через новый метод без пробелов
        if hasattr(self.mw, 'chat_handler'):
            self.mw.chat_handler.append_html(f"<span style='color: #569cd6;'>{trace_link} [{mode_notice}]: {user_text}</span><br>")
            self.mw.chat_handler.scroll_chat()
        else:
            self.mw.chat_history.append(f"<div style='margin-top: 2px; margin-bottom: 2px;'><span style='color: #569cd6;'>{trace_link} [{mode_notice}]: {user_text}</span></div>")
            self.mw.scroll_chat()
            
        self.mw.tokens_sent += self.mw.ai_controller.estimate_tokens(full_prompt)
        self.mw.update_status_bar()

        # 4. РАЗДЕЛЕНИЕ: Очищаем текст от управляющих символов @ и [] БЕЗ удаления имен файлов
        # Паттерн находит @[имя_файла] и заменяет его на внутреннюю группу (чистое имя_файла)
        clean_clipboard_text = re.sub(r'@\[([^\]]+)\]', r'\1', user_text).strip()
        QApplication.clipboard().setText(clean_clipboard_text)

        # 5. ПОДГОТОВКА ФАЙЛОВ ДЛЯ DRAG 
        ...
        # Б) Любые прикрепленные файлы из множества UI-состояния (Официальные вложения с кодом)
        if hasattr(self.mw, 'attached_files') and self.mw.attached_files:
            for file_tag in self.mw.attached_files:
                add_file_url(file_tag)

        # В) Технический файл с тяжелым контекстом (RAG, деревья)
        ...
        log_func("Ваш запрос скопирован. Бросьте файлы -> Ctrl+V -> Enter.", color="#31a24c")
        ...
        # ПРОГРАММА БЛОКИРУЕТСЯ ЗДЕСЬ ДО ОКОНЧАНИЯ ПЕРЕТАСКИВАНИЯ
        result = drag.exec(Qt.DropAction.CopyAction)
        
        # Взводим радар только ПОСЛЕ того, как файлы физически упали в браузер!
        if hasattr(self.mw.ai_controller, 'activate_radar'):
            self.mw.ai_controller.activate_radar()
